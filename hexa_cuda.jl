...
function compute_hexa_reset_main(n::Int, probability_main::Array, cost_data::Array,
...
for (i, reset_at_attempt) in enumerate(10:19)
    ...
    for reset_below_level in reset_below_level_minimum:(goal_level-1)
        println("-"^50)
        println("Goal for main = ", goal_level)
        println("Reset at attempt = ", reset_at_attempt)
        println("Reset when main below level = ", reset_below_level)

        # Approximate number of resets to tune number of attempts
        main_cuda, secondary_1_cuda, secondary_2_cuda, 
        cost_cuda, reset_cuda = compute_hexa_reset_main(1024, probability_main, cost_data, 
        ...
        main_cuda, secondary_1_cuda, secondary_2_cuda, 
        cost_cuda, reset_cuda = compute_hexa_reset_main(2^magnitude, probability_main, cost_data, 
                                                        goal_level, reset_at_attempt, reset_below_level,
                                                        n_threads);
...
writedlm("./$(folder_results)/summary_cost_mean_$(goal_level).txt", summary_cost_mean)
writedlm("./$(folder_results)/summary_cost_50_$(goal_level).txt", summary_cost_50)
writedlm("./$(folder_results)/summary_cost_90_$(goal_level).txt", summary_cost_90)
writedlm("./$(folder_results)/summary_cost_95_$(goal_level).txt", summary_cost_95)
writedlm("./$(folder_results)/summary_cost_99_$(goal_level).txt", summary_cost_99)

writedlm("./$(folder_results)/summary_reset_mean_$(goal_level).txt", summary_reset_mean)
writedlm("./$(folder_results)/summary_reset_50_$(goal_level).txt", summary_reset_50)
writedlm("./$(folder_results)/summary_reset_90_$(goal_level).txt", summary_reset_90)
writedlm("./$(folder_results)/summary_reset_95_$(goal_level).txt", summary_reset_95)
writedlm("./$(folder_results)/summary_reset_99_$(goal_level).txt", summary_reset_99)
